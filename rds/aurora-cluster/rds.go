...
func NewRDSStack(scope constructs.Construct, props *awscdk.StackProps) awscdk.Stack {
	...
	subnetGroup := rds.GetPrivateSubnetGroup(stack, cfg.GetQualifiedName("SubnetGroup"), vpc)

	sgs := &[]awsec2.ISecurityGroup{
		rds.GetAllowAllVPCSecurityGroup(stack, cfg.GetQualifiedName("SG"), vpc),
	}
